...
__attribute__((nonnull(1,2))) static inline void rb_get_state(volatile lap_ptr_rb_header_t *const rb_in,  lap_ptr_rb_state_t *const restrict state) {
    ...
    size_t ptr_xor_mask = (size | idx_mask) & (rd_ptr ^ wr_ptr); // {(N+1){1'b1}} & (wr_ptr[N:0] ^ rd_ptr[N:0])
 ...
 */
__attribute__((nonnull(1))) static inline void rb_advance_write_unsafe(volatile lap_ptr_rb_header_t *const rb, size_t n) {
    rb->_wr_ptr += n;
};
__attribute__((nonnull(1))) static inline void rb_advance_read_unsafe(volatile lap_ptr_rb_header_t *const rb, size_t n) {
    rb->_rd_ptr += n;
};
